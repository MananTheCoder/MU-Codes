...
vvb get_adj_matrix_ud_uw(pivvi &graph)
{
    // undirected
    int v = graph.first;
...
mivi convert_adj_list_uw_ud(vvb &adj_matrix)
{
...
void solve()
{
    ...
    vvb adj_matrix = get_adj_matrix_ud_uw(graph);
    print_vvb(adj_matrix);
    mivi adj_list = convert_adj_list_uw_ud(adj_matrix);
    print_mivi(adj_list);
